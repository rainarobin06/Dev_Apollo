    ...
    $rootScope.getCurrentUserInfo = function () {
        $http({
            ...
            if ($rootScope.currentUserObject.user_detail[0].module === 'No') {
                alert('Access denied. Your account does not configured to use the Nursing Portal, please reach out lighthouse admin for the access!');
    ...
    $scope.ValidateFrom = function () {
        ...
        switch ($scope.ReportSelected.Formcode) {
            ...
            case "TBL06" /*Top 5 Incidents*/:
                angular.forEach($scope.form.topincidents, function (item) {
                    if (!item.NameOfIncident || !item.NumberOfIncidents || !item.ActionTaken || !item.MedicationError || !item.NSI || !item.PatientFall || !item.SampleError) {
                        $scope.IsValidForm = false;
                    }
                });
                break;
        ...
        $http({
            ...
            if (userResult.status.StatusCode != "200") {
                ...
                if (userResult.user_detail[0].module === 'No') {
                    $rootScope.DisaplyMessage('danger', "This account does not have access to Nursing Portal in lighthouse. Enable access and try add again.");
                    $scope.IsAdding = false;
                } else {
